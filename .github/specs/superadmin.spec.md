...
|--------|--------|--------------|
| **RN-05: Permisos de gestión** | SUPER_ADMIN acceso global. STORE_ADMIN acceso limitado a su ecommerce. STORE_USER acceso a perfil propio. | Modelo multi-tenant: cada STORE_ADMIN gestiona su marca sin poder tocar otras. Usuarios pueden cambiar su propia contraseña. |
| **Endpoint PUT** | Permite actualización por SUPER_ADMIN (global), STORE_ADMIN (su ecommerce), Usuario (perfil propio con restricciones). | Usuario puede cambiar contraseña. STORE_ADMIN gestiona staff. Solo SUPER_ADMIN puede cambiar ecommerce_id. |
| **Endpoint DELETE** | Permite eliminación por SUPER_ADMIN (global), STORE_ADMIN (su ecommerce). | STORE_ADMIN puede eliminar usuarios de su marca. |
| **Endpoint POST** | SUPER_ADMIN: crea cualquier rol en cualquier ecommerce. STORE_ADMIN: crea STORE_USER/STORE_ADMIN en su ecommerce. | STORE_ADMIN gestiona su staff. Control tenant-scoped. |
| **Endpoint GET /ecommerces** | Solo SUPER_ADMIN accede a lista global. Otros retornan 403. | Solo administrador plataforma ve todos los ecommerces. |
| **SecurityContextHelper** | Nuevos métodos: `isCurrentUserSuperAdmin()`, `getCurrentUserEcommerceId()`, `canActOnUser()`. | Support para validaciones de autorización multi-contexto. |
| **UserEntity** | CHECK constraint (SUPER_ADMIN + ecommerce_id=NULL) | Garantizar integridad referencial. |
| **Frontend** | Crear `/superadmin` pages con gestión de usuarios y ecommerces. | Interfaz dedicada para administrador plataforma. |
| **Frontend** | Proteger rutas `/superadmin/*` + crear vista limitada para STORE_ADMIN. | Access control basado en JWT role. |

---

## Criterios de Aceptación Global (Spec)

✅ **Funcionales:**
- SUPER_ADMIN puede crear STORE_ADMIN para ecommerce.
- SUPER_ADMIN puede listar todos los ecommerces sin filtro.
- SUPER_ADMIN puede actualizar datos de usuarios (username, password, ecommerce, status).
- SUPER_ADMIN puede eliminar usuarios (con protección de auto-eliminación).
- SUPER_ADMIN puede reasignar usuarios entre ecommerces.
- **STORE_ADMIN puede crear STORE_USER y STORE_ADMIN dentro de su ecommerce.**
- **STORE_ADMIN puede actualizar usuarios dentro de su ecommerce.**
- **STORE_ADMIN puede eliminar usuarios dentro de su ecommerce.**
- **Cualquier usuario puede cambiar su propia contraseña (sin cambiar ecommerce_id).**
- STORE_ADMIN/STORE_USER NO pueden hacer operaciones fuera de su ecommerce (403 Forbidden).
- STORE_USER NO puede crear/actualizar/eliminar usuarios (403 Forbidden).

✅ **Técnicos:**
- CHECK constraint en BD garantiza SUPER_ADMIN.ecommerce_id = NULL.
- JWT include claim `role: SUPER_ADMIN`, `role: STORE_ADMIN`, etc.
- Validación de autorización en cada endpoint (SUPER_ADMIN global, STORE_ADMIN tenant-scoped, STORE_USER self-limited).
- Helper `SecurityContextHelper.canActOnUser()` valida acceso contextual.
- Logging de auditoría en cada operación CRUD con actor + target + contexto.

✅ **Seguridad:**
- Username único globalmente.
- Contraseña fuerte (12+ chars, BCrypt).
- Validación de autorización en CADA endpoint (no solo endpoint-level, sino object-level).
- Aislamiento multi-tenant respetado: STORE_ADMIN no puede ver/editar/eliminar usuarios de otros ecommerces.
- STORE_USER no puede acceder operaciones administrativas (GET lista usuarios, DELETE, etc.).
- User NO puede cambiar su propio ecommerce_id (validación en PUT).
- Contraseña NO se retorna en responses (solo uid/username/role).
- Token JWT válida para 24h (valor por defecto).
- DELETE usuario no falla silenciosamente; retorna 204 o error explícito.
