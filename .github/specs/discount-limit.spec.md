...
| `allow_stacking` | BOOLEAN | sí | default TRUE | Permite apilar descuentos |
...
|-------|------|-------------|------------|-------------|
| `id` | UUID | sí | auto-generado (gen_random_uuid()) | Identificador único |
| `discount_setting_id` | UUID | sí | FK → discount_settings.id | Referencia a la configuración |
| `discount_type_id` | UUID | sí | FK → discount_types.id | Tipo de descuento |
| `priority_level` | INTEGER | sí | > 0 | Orden de aplicación (1 = máxima prioridad) |
| `created_at` | TIMESTAMP WITH TIME ZONE | sí | auto-generado | Timestamp creación |
| `updated_at` | TIMESTAMP WITH TIME ZONE | sí | auto-generado | Timestamp última actualización |
...
- **PRIMARY KEY**: `id` (UUID)
- **UNIQUE PARCIAL**: `(ecommerce_id, is_active)` WHERE `is_active = true`
- **FK**: `ecommerce_id` → `ecommerce(id)` ON DELETE CASCADE
- **INDEX**: `idx_discount_settings_ecommerce` (ecommerce_id)
- **CHECK**: `max_discount_cap > 0`

*Tabla `discount_priorities`*
- **PRIMARY KEY**: `id` (UUID)
- **FK**: `discount_setting_id` → `discount_settings(id)` ON DELETE CASCADE
- **UNIQUE**: `(discount_setting_id, discount_type_id)`
- **UNIQUE**: `(discount_setting_id, priority_level)`
- **INDEX**: `idx_discount_priorities_setting` (discount_setting_id)
